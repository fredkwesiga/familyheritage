...
import { useRequestMagicLink } from '@/features/auth/use-auth';

/** Requests a one-time sign-in link. The click-through lands on MagicLinkCallbackPage. */
export function MagicLinkPage() {
  ...
  const onSubmit = handleSubmit(async (values) => {
    setFormError('');
    try {
      await requestLink.mutateAsync(values);
  ...
  // an account. Saying "no account found" would confirm to anyone who asks
  // whether a given person is documenting their family.
  if (sentTo) {
    return (
      <div className="space-y-6">
        ...
        <header className="space-y-2">
          ...
          <p className="text-muted-foreground">
            If an account exists for <span className="text-foreground">{sentTo}</span>, a sign-in
            link is on its way. It works once and expires shortly.
          </p>
        </header>
  ...
  return (
    <div className="space-y-8">
      <header className="space-y-2">
        <h1 className="font-serif text-3xl tracking-tight">Sign in without a password</h1>
        <p className="text-muted-foreground">
          We'll email you a link. Nothing to remember, nothing to type.
        </p>
      </header>
